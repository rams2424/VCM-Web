...
public class ViewAccountActivityPage extends BasePage {
    ...
    public void clickOnCancelTransaction(){
        try{
            Assert.assertTrue(cancelTransaction.isDisplayed());
        }
        catch(Exception e){
            Assert.assertTrue(false,"Cancel is not available");
        }
    ...
    @FindBy(xpath="//div[not(contains(@tabindex,'-1'))]/button[@class='sc-likbZx sc-drMfKT icAqXC'][2]")
    WebElement futureDateBttn;
    ...
    public void setFutureStartDate(){
        this.clickOnButton(tranActivityStartDate);
        this.clickOnButton(futureDateBttn);
    ...
    public void setFutureEndDate(){
        this.clickOnButton(tranActivityEndDate);
        this.clickOnButton(futureDateBttn);
        this.clickOnButton(okBttn);		
    }
    ...
    public void verifyTransactionIsCancelled(String amount,String expected) {
        String date = this.getCurrentDate();
        log.info("Current Date"+date);
        log.info("Amount"+amount);
        log.info("Expected"+expected);
        List<WebElement>list =driver.findElements(By.xpath("//span[text()='"+date+"']/../following-sibling::td[3]//span[contains(text(),'$"+amount+"')]/../following-sibling::td//span"));
        log.info("Actual"+list.get(0).getText());
        Assert.assertTrue(expected.equalsIgnoreCase(list.get(0).getText()));
    }
    @FindBy(xpath="//div[contains(text(),'Cancel')]")
    WebElement cancelBtn1;
    @FindBy(xpath="//span[contains(text(),'Yes')]")
    WebElement yesBtn;
    public void cancelTransaction(String fundSource)  {
        log.info("Fund source"+fundSource);
        try {
            if(fundSource.equalsIgnoreCase("bank Account") && driver.findElement(By.xpath("//div[contains(text(),'Cancel')]")).isDisplayed()) {
                wait = new WebDriverWait(driver, 40);
                wait.until(ExpectedConditions.elementToBeClickable(cancelBtn1));
                cancelBtn1.click();
            ...
            try {
                this.takeSnapShot(driver, this.getScreenShotFilePath()+"cancelTransaction.png");
            } catch (Exception e1) {
